...
fn parse(input: &str) -> Vec<Region> {
    ...
    regions.lines().map(|line| {
        ...
        let presents = right.split_ascii_whitespace().map(|num| num.parse::<u64>().unwrap()).collect();

        Region { width: width.parse().unwrap(), height: height.parse().unwrap(), presents }
    }).collect()
}
...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    let total = regions.iter().map(|region| {
        let mut sub_value = 0;
